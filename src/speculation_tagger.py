...
class SpeculationTagger(object):
    ...
    def __call__(self, sentence_instances, outfile=''):

        tagged_sentences = []
        tagged_sentences_ids = []
        for file_id in tqdm(sentence_instances):
            for sentence_id in sentence_instances[file_id]:
                sentence_concept_instances = sentence_instances[file_id][sentence_id]
    ...
    def tag_modality_cues(self, sentence_tags, metadata, sentence):
        ...
        modality_triggers = self.speculation_cues
        ...
        # make sure that no token is annotated with more than one speculation instance
        ...
        for (modality_cue, match_idxs) in modality_matches:
            modality_tag = 'speculation_{}'.format(len(metadata))
            metadata['speculation'][modality_tag] = {'cue': modality_cue}
            for i in match_idxs:
