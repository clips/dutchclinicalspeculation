...
class PreprocessCorpus:
    ...
    def __call__(self, file_ids, already_frogged=False, corpusfile=False, outfile=''):
        ...
        for file_id in tqdm(file_ids):
            try:
                ...
                print('Corrupted data encountered, ignoring and carrying on...')

        grouped_sentences = self.group_sentences(accumulated_concept_instances)
    ...
    def convert_corpusfile_to_general_format(data):
        ...
        general_format['speculation_status'] = []

        modality = data['modifications']
        negated_concepts = [x['obj'] for x in modality if x['pred'] == 'Speculation']

        for concept_dict in data['denotations']:
            span = concept_dict['span']
            if concept_dict['id'] in negated_concepts:
                speculation_status = True
            else:
                speculation_status = False
            general_format['concept_spans'].append(span)
            general_format['speculation_status'].append(speculation_status)
    ...
    def extract_instances(self, data, frogged_text, file_id):
        ...
        for i, concept_span in enumerate(concept_spans):
            ...
            if not sentence:
                ...
                continue
            if 'speculation_status' in data:
                modality = data['speculation_status'][i]
    ...
    @staticmethod
    def convert_corpus_frog_output(frogged_output):
        converted_sentences = []
        current_sentence = []
